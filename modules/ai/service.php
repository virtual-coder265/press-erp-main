...
if (!function_exists('ai_detect_feature')) {
    function ai_detect_feature($message, $requestedFeature = '') {
        ...
        $patterns = [
            'invoices' => [
                '/\b(invoice|invoices|invoicing|receivable|receivables|unpaid|overdue|payment|payments|collection|collections|outstanding|customer balance|aged debt|aging)\b/' => 3,
                '/\b(balance due|past due|due date|paid amount|cash collection)\b/' => 2,
            ],
            'estimations' => [
                '/\b(estimation|estimations|estimate|estimates|quotation|quotations|quote|quotes|costing|job costing|profit margin|margin|labou?r cost|materials cost|paper cost)\b/' => 3,
                '/\b(pricing|approval|conversion|cost breakdown)\b/' => 2,
            ],
            'sales' => [
                '/\b(sale|sales|revenue|turnover|customer demand|sales pipeline|sales performance|sold|direct sale|direct sales)\b/' => 3,
                '/\b(top customer|top customers|product performance|service performance|collections trend)\b/' => 2,
            ],
            'tasks' => [
                '/\b(task|tasks|to do|todo|assignment|assignments|work item|work items|action item|action items|pending work|backlog)\b/' => 3,
                '/\b(priority|priorities|blocker|blockers|next task|next tasks)\b/' => 2,
            ],
            'projects' => [
                '/\b(project|projects|milestone|milestones|timeline|timelines|delivery plan|deliveries|project progress|completion rate)\b/' => 3,
                '/\b(project risk|deadline|deadlines|project status)\b/' => 2,
            ],
            'reminders' => [
                '/\b(reminder|reminders|alarm|alarms|follow up|follow-up|schedule|scheduled|upcoming|notify me|notification)\b/' => 3,
                '/\b(remind|calendar|due soon|follow-up)\b/' => 2,
            ],
        ];

        $priority = ['invoices', 'estimations', 'sales', 'tasks', 'projects', 'reminders'];
        $scores = [];
        foreach ($patterns as $feature => $featurePatterns) {
            $scores[$feature] = 0;
            foreach ($featurePatterns as $pattern => $weight) {
                $matches = preg_match_all($pattern, $text, $buffer);
                if ($matches) {
                    $scores[$feature] += ((int) $matches * (int) $weight);
                }
            }
        }

        $bestFeature = 'analysis';
        $bestScore = 0;
        foreach ($priority as $feature) {
            $score = (int) ($scores[$feature] ?? 0);
            if ($score > $bestScore) {
                $bestScore = $score;
                $bestFeature = $feature;
            }
        ...
        return $bestScore > 0 ? $bestFeature : 'analysis';
    }
}
...
if (!function_exists('ai_make_context')) {
    function ai_make_context($feature, $scope = 'user') {
        return [
            'summary' => [],
            'items' => [],
            'meta' => [
...
if (!function_exists('ai_context_add_warning')) {
    function ai_context_add_warning(array &$context, $warning) {
        ...
        $context['meta']['partial'] = true;
        if (($context['meta']['integrity'] ?? 'ok') === 'ok') {
            $context['meta']['integrity'] = 'warning';
        }
    }
}

if (!function_exists('ai_finalize_context')) {
    function ai_finalize_context(array $context, $itemLimit = 0) {
        $context['summary'] = isset($context['summary']) && is_array($context['summary']) ? $context['summary'] : [];
        $context['items'] = isset($context['items']) && is_array($context['items']) ? $context['items'] : [];
        $context['warnings'] = isset($context['warnings']) && is_array($context['warnings']) ? array_values(array_unique(array_map('strval', $context['warnings']))) : [];
        $context['meta'] = isset($context['meta']) && is_array($context['meta']) ? $context['meta'] : [];
        $context['meta']['generated_at_utc'] = isset($context['meta']['generated_at_utc']) ? (string) $context['meta']['generated_at_utc'] : gmdate('c');
        $context['meta']['item_limit'] = max(0, (int) $itemLimit);
        $context['meta']['item_count'] = count($context['items']);
        $context['meta']['empty'] = empty($context['summary']) && empty($context['items']);

        if (!empty($context['warnings']) && ($context['meta']['integrity'] ?? 'ok') === 'ok') {
            $context['meta']['integrity'] = 'warning';
        }

        if ($context['meta']['empty']) {
            ai_context_add_warning($context, 'No matching ERP records were available for this context snapshot.');
        }
...
if (!function_exists('ai_user_can_access_feature_data')) {
    function ai_user_can_access_feature_data($feature) {
        $feature = strtolower((string) $feature);
        $permissionMap = [
            'tasks' => 'view_tasks',
        ...
        $permission = $permissionMap[$feature];
        if ($permission === null || !function_exists('hasPermission')) {
            return true;
        }
...
if (!function_exists('ai_fetch_invoices_context')) {
    function ai_fetch_invoices_context(PDO $pdo, $userId) {
        $context = ai_make_context('invoices', 'organization');
        $itemLimit = 8;
        try {
            ...
            ");
            $itemsStmt->execute();
            $context['items'] = $itemsStmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
        } catch (Exception $e) {
            ai_context_add_warning($context, 'Invoice context fetch warning: ' . $e->getMessage());
        }

        return ai_finalize_context($context, $itemLimit);
    }
}

if (!function_exists('ai_fetch_estimations_context')) {
    function ai_fetch_estimations_context(PDO $pdo, $userId) {
        $context = ai_make_context('estimations', 'organization');
        $itemLimit = 8;
        try {
            $summaryStmt = $pdo->prepare("
                SELECT
            ...
            ai_context_add_warning($context, 'Estimation context fetch warning: ' . $e->getMessage());
        }
...
if (!function_exists('ai_fetch_sales_context')) {
    function ai_fetch_sales_context(PDO $pdo, $userId) {
        $context = ai_make_context('sales', 'organization');
        $itemLimit = 8;
        try {
            $summaryStmt = $pdo->prepare("
                SELECT
            ...
            ai_context_add_warning($context, 'Sales context fetch warning: ' . $e->getMessage());
        }
...
if (!function_exists('ai_fetch_feature_context')) {
    function ai_fetch_feature_context(PDO $pdo, $userId, $feature) {
        $userId = (int) $userId;
        $feature = strtolower((string) $feature);
        $context = ai_make_context($feature, 'user');
        ...
        try {
            if ($feature === 'tasks') {
                ...
                if (!ai_user_can_access_feature_data('invoices')) {
                    ai_context_add_warning($context, 'Invoice context is unavailable because the current user does not have invoice access.');
                    return ai_finalize_context($context);
                }
                return ai_fetch_invoices_context($pdo, $userId);
            } elseif ($feature === 'estimations') {
                if (!ai_user_can_access_feature_data('estimations')) {
                    ai_context_add_warning($context, 'Estimation context is unavailable because the current user does not have estimation access.');
                    return ai_finalize_context($context);
                }
                return ai_fetch_estimations_context($pdo, $userId);
            } elseif ($feature === 'sales') {
                if (!ai_user_can_access_feature_data('sales')) {
                    ai_context_add_warning($context, 'Sales context is unavailable because the current user does not have sales access.');
                    return ai_finalize_context($context);
                }
                return ai_fetch_sales_context($pdo, $userId);
            } else {
                $itemLimit = 0;
                ...
                if (ai_user_can_access_feature_data('invoices')) {
                    ...
                    $context['summary']['unpaid_invoices'] = (int) $invoicesStmt->fetchColumn();
                } else {
                    ai_context_add_warning($context, 'Financial invoice counts were excluded from analysis context because the current user does not have invoice access.');
                }

                if (ai_user_can_access_feature_data('estimations')) {
                    $estimationsStmt = $pdo->prepare("SELECT COUNT(*) FROM estimations WHERE status != 'Invoiced'");
                    $estimationsStmt->execute();
                    $context['summary']['pending_estimations'] = (int) $estimationsStmt->fetchColumn();
                } else {
                    ai_context_add_warning($context, 'Estimation counts were excluded from analysis context because the current user does not have estimation access.');
                }

                $context['meta']['scope'] = 'mixed';
                return ai_finalize_context($context, $itemLimit);
            }
        } catch (Exception $e) {
            ai_context_add_warning($context, 'Context fetch warning: ' . $e->getMessage());
        }
...
if (!function_exists('ai_build_system_prompt')) {
    function ai_build_system_prompt($feature) {
        ...
        return implode("\n", [
            ...
            'Always analyze and reference the provided ERP context. It includes summary data, item snapshots, metadata, and warnings.',
            'Treat generated_at_utc as the freshness timestamp for the context snapshot.',
            'If warnings are present or meta.integrity is not "ok", explicitly mention the limitation and avoid overstating certainty.',
            'If summary or items are empty, say the system snapshot did not return enough matching data instead of inventing an answer.',
...
if (!function_exists('ai_chat')) {
    ...
    function ai_chat(PDO $pdo, $userId, $message, $requestedFeature = '') {
        ...
        $feature = ai_detect_feature($rawMessage);
        ...
        if ($contextJson === false) {
            $contextJson = json_encode(['error' => 'Context serialization failed.']);
        }
        $contextMessage = "ERP CONTEXT DATA (real system snapshot):\n" . $contextJson;
